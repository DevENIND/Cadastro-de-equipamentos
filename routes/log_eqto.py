...
def enviar_email():
    ...
    if nome_btn == 'Enviar':
           ...
           email_completo = prefixoemail + email
           Validacao, Valor_Erro = funcoes.emails.valida_codigo(email_completo,codigo)
           if Validacao == True:
               return redirect('/Ctr_Eqto')
           else:
...
@log_eqto_route.route('/reenvio', methods=['GET','POST'])
def reenviar_email():
    
    prefixoemail = request.values.get("CmplEmail")
    email = request.values.get("email")
    codigo = request.values.get("Codigo")
    nome_btn =  request.values.get("btnEnviar")
